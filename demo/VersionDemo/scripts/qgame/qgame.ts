        ...
        if (file.extname == '.js') {
            ...
            if (filename == 'libs/modules/egret/egret.js' || filename == 'libs/modules/egret/egret.min.js') {
                ...
                if (filename.indexOf('egret3d.js') >= 0) {
                    content = `var RES = window.RES; const system = require("@system");` + content;
                    content = content.replace(new RegExp('egret.web', 'g'), "egret.miqgame");
                }
                else if (filename.indexOf("egret3d.min.js") >= 0) {
                    content = `var RES = window.RES; const system = require("@system");` + content;
                    content = content.replace(new RegExp('egret.web', 'g'), "egret.miqgame");
                    content = content.replace(new RegExp('e.webgl', 'g'), "AAABBBCCC");
                    content = content.replace(new RegExp('e.web', 'g'), "e.miqgame");
                    content = content.replace(new RegExp('AAABBBCCC', 'g'), "e.webgl")
                }
                if (filename == 'main.js') {
